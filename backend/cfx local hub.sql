...
REFERENCES User_(user_id) ON DELETE CASCADE;
...
REFERENCES Trader(trader_id) ON DELETE CASCADE;
...
REFERENCES ProductCategory(category_id) ON DELETE CASCADE;
...
CREATE TABLE CollectionSlot(
    ...
    time_slot VARCHAR2(15) NOT NULL,
...
REFERENCES CollectionSlot(collection_slot_id) ON DELETE CASCADE;
...
REFERENCES OrderDetail(order_id) ON DELETE CASCADE;
...
REFERENCES PaymentType(payment_type_id) ON DELETE CASCADE;
